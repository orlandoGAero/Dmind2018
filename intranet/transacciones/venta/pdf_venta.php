...
<?php
    ...
    $idVenta = base64_decode($_GET['vent']);
    $fecha = base64_decode($_GET['dt']);
    $nomStVentas = base64_decode($_GET['st']);
    $numCliente = base64_decode($_GET['cl']);
    $IDtipVenta = base64_decode($_GET['tv']);
    $IDformaPago = base64_decode($_GET['fp']);
    $moneda = base64_decode($_GET['m']);
    $impuesto = base64_decode($_GET['i']);
    $tipoImpuesto = base64_decode($_GET['ti']);
...
<page backtop="10mm" backbottom="10mm" backleft="10mm" backright="10mm" pagegroup="new">
    <page_header>
        ...
        <div class="divstatus">
            <b><?=$funcVentas -> obtnerNombreStatusVenta($nomStVentas)?></b>
        </div>
    </page_header>
        ...
        <tr>
            ...
            <td colspan="3" class="borderAzul">
                <table id="datosCliente">
                    <?php $datcl = $funcVentas -> datosCliente($numCliente); ?>
                    <tr><td><div style="width: 95%;">Cliente: <b><?=$funcVentas -> obtenerNombreCliente($numCliente)?></b></div></td></tr>
            ...
            <td colspan="3" class="borderAzul">
                <table id="datosVendedor">
                    <tr><td>Vendedor: <b>EMANUEL ESPEJEL RODRIGUEZ</b></td></tr>
                    <tr><td class="br"></td></tr>
    ...
    <?php $datosdetv = $funcVentas -> tmpObtenerDetalleVenta($idVenta) ?>
    <table id="tablaProductos">
      ...
      <?php foreach($datosdetv as $datdetve) :?>
          <tr>
            <td class="minimum"><?=$datdetve['cantProd']?></td>
            <td class="maximum descripcion">
                <?php $notaProducto = explode('[br]', $datdetve['notaDescr']) ?>
                <b><?=$datdetve['nombre']?></b>
            ...
            <td class="minimum">
                <b>$</b><?=number_format($datdetve['precio_unitario_tmp'],2,'.',',')?>
            </td>
            <?php $import = $datdetve['precio_unitario_tmp']*$datdetve['cantProd'] ?>
            <td class="minimum">
    ...
    <table cellspacing="-1" id="operaciones">
        <tr>
            <td class="izqOperac">
                ...
                <?php $nomTipVenta = $funcVentas -> obtenerNombreTipoVenta($IDtipVenta); ?>
                Tipo de venta: <b><?=$nomTipVenta?></b>&nbsp;
                <?php $nomFormaPago = $funcVentas -> obtenerNombreFormaPago($IDformaPago); ?>
                Forma de pago: <b><?=$nomFormaPago?></b>
            </td>
            <td class="azulOperac radiusBottomIzq" rowspan="3">
                <?php $nombreImpuesto = $funcVentas -> obtenerNombreImpuesto($tipoImpuesto); ?>
                SUBTOTAL:<br />
            ...
            <td class="costos radiusBottomDer" rowspan="3">
                ...
                <b>$</b><?=number_format($SubtotalVenta,2,'.',',')?>
                <br />
                <!-- IMPUESTO -->
                <?=$impuesto?>%
                <br />
                <!-- Total -->
                <?php $ivaDecimales = ($impuesto/100) ?>
                <?php $calculandoTotal = ($SubtotalVenta * $ivaDecimales) ?>
                <?php if($tipoImpuesto == 1 || $tipoImpuesto == 2) :?>
                    <?php $resultadoTotal = ($SubtotalVenta + $calculandoTotal) ?>
                <?php elseif($tipoImpuesto == 3) :?>
                    <?php $resultadoTotal = ($SubtotalVenta - $calculandoTotal) ?>
                <?php endif; ?>
                <b>$</b><?=number_format($resultadoTotal,2,'.',',')?>
            </td>
        </tr>
        <tr>
            <td class="izqOperac">
