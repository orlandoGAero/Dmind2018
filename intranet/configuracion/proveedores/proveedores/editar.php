...
$query="SELECT * FROM proveedores WHERE id_proveedor=$id_prov";
...
<input type="hidden" name="id_prov" value="<?php echo $id_prov ?>" readonly />
	<b>Nombre:</b>
		<input name="nom_clie" type="text" value="<?php echo $fila[1] ?>" />
<b>Teléfono:</b>
<input type="text" name="telefono" value="<?php echo $fila["telefono"]; ?>" /><br><br>
	<b>Categoría:</b>
	<select name="id_categoria_prov"><br>
<?php $id_cat=$fila["id_cat_prov"]; 
}
?>
<?php 
$query="SELECT * FROM categorias_proveedor where id_cat_prov=$id_cat";
$result=mysql_query($query);
while($fila=mysql_fetch_array($result)){
	echo "<option value='",$fila[0],"'>",$fila[1],"</option>";
}

 $query="SELECT * FROM categorias_proveedor where not id_cat_prov=$id_cat";
$result=mysql_query($query);
while($fila=mysql_fetch_array($result)){
	echo "<option value='",$fila[0],"'>",$fila[1],"</option>";
}
...
<hr> 
<?php
$query="SELECT C.id_datfiscal,razon_social,rfc,tipo_razon_social,C.id_contacto,D.id_direccion,C.id_bancarios FROM proveedores C
		inner join datos_fiscales D on C.id_datfiscal=D.id_datfiscal
		inner join direcciones on C.id_direccion=direcciones.id_direccion WHERE id_proveedor=$id_prov";
$result=mysql_query($query);
while($fila=mysql_fetch_array($result)){
 ?>
...
<td>
		Tipo Cuenta:<input type="text" name="tipo_cuenta" value="<?php echo $fila["tipo_cuenta"] ?>">
</td>
</tr>
</table>
		<hr>
...
<center>
<?php
$query="SELECT C.id_prov_cont,nombre,movil,nombre_areacontacto,C.id_contacto from proveedorxcontacto C 
...
<?php 
$query="select sum(id_proveedor) from proveedorxcontacto where id_proveedor=$id_prov";		
$result=mysql_query($query);
while($fila=mysql_fetch_array($result)){
	$nu=$fila[0];
}
...
<div id="nuevocont" style="display:none">
<br>
<br><br>
<br>
<br>
...
while($fila=mysql_fetch_array($result)){
  ...
  echo "<td align='center'>", $fila[3]; "</td>";
  echo "<td align='center'>", $fila[4]; "</td>";
  echo "<td><a href='actializarcontacto.php?id_cont=";
  echo $fila[0];
  echo "&";
  echo "id_prov=";
  echo $_GET["id_prov"];
