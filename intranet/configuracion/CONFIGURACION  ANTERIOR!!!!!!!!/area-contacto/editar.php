...
<form action="actualiza.php" class="regusuario"> 
	<h2 class="formarriba">EDITAR AREA CONTACTO</h2>
<div class="camps">
	<br><label>Nombre :</label>
<?php
  $id_areacontacto=$_GET["id_areacontacto"];
  include ('../../Conexion.php');
  $query = "select * from areacontacto where id_areacontacto=$id_areacontacto";
  $result=mysql_query($query);
  while($fila=mysql_fetch_array($result))  {
  	echo '<input type="hidden" name="id_areacontacto" value="'.$fila["id_areacontacto"].'" /><br><br>';
  	echo '<input name="nombre_areacontacto" value="'.$fila["nombre_areacontacto"].'" type="text"/><br><br><br>';
  }
?>
</div>
